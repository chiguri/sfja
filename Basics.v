...
*)
(** まず最初はとても簡単なサンプルから始めましょう。
    次の定義は、Coqに対して、新しいデータ型のセット（集合）である「型(type)」を定義しています。 *)
...
(** 一つ注意しておかなければならないことがあります。
    ...
    それはいわゆる「型推論」という機構によって実現されていますが、型を明示した方がプログラムを読みやすくできると判断するなら、いつでもそうしてかまいません。 *)
...
*)
(** 三番目の方法は、Coqで[定義]したものから、他の一般的な言語（OCamlやScheme、Haskell）のプログラムを抽出してしまうことです。
    この機能は今主流の言語で完全に確認されたプログラムを実現できる道を開いたという意味でとても興味深いものです。
    これはCoqの開発動機の一つです。
...
(** このように我々は独自のbool型を一から作っていますが、もちろんCoqには標準ライブラリとしてbool型が多くの有用な関数、補題と一緒に用意されています。
    ...
    ここでは可能な限り標準ライブラリと正確に同じ機能を、我々独自の名前で定義していくことにしましょう。 *)
...
(** [negb] のような関数は、それ自身が [true] や [false] と同じように値であると考えることもできます。
    そのようにとらえた場合の値の型を「関数型」と呼び、以下のように矢印を使った型として表します。 *)
...
*)
(** [negb]の型を[bool -> bool]と書き、「[bool]から[bool]」と読み、[bool]型の引数をとって[bool]型の戻り値を返す関数と理解することができます。
    同様に、[andb]の型は[bool -> bool -> bool]と書き、「二つの[bool]型の値を引数として受け取ることで、この関数は[bool]型の値を作成して出力する」と解釈します。 *)
...
(** ちょっと技術的な話：Coqは大規模な開発を支援するためにちょっと大げさにも見えるモジュールシステムを提供しています。
    このコースではこれらはほとんど必要のないものですが、一つだけ有用なものがあります。
    プログラムの中のいくつかの要素を[Module X]と[End X]で囲んでおくと、[End X]以降の部分から、囲まれた中の定義を[X.foo]という風に呼び出すことができます。
    このことは、新しく[foo]という名前で関数を定義しても問題ないということです。
    逆に、同じスコープの中では、同じ名前での定義はエラーとなります。
    という訳で、今回我々はこの機能を使って[nat]という型を内部モジュールとして定義します。
...
(** 我々がここまでで定義してきた型は「列挙型」の型定義でした。
    ...
    型を定義するもう一つの方法は、「帰納的な記述」を並べることで要素を記述する方法です。
    例えば、自然数は（全て並べるわけにはいきませんが）以下のような方法で定義できます。 *)
...
(** この定義の各句は、以下のように解釈できます。
    ...
    これまでに定義してきた帰納的な型（[weekday]、[nat]、[bool]など）は、実際には式の集合とでも言うべきものです。
...
(** 自然数というのは非常に一般的な型なので、Coqは自然数を扱ったり表したりするときに若干特別な扱いをします。
    [S]や[O]を使った式の代わりに一般的に使われるアラビア数字を使うことができます。
...
(** これらが表しているのは、いずれの関数も数を引数にとって数を生成できる、ということです。
    ...
    [pred]や[minustwo]といった関数には「計算ルール」というものが定義されています。
    [pred]の定義は、[pred n]が[match n with | O => O | S m' => m' end]のように簡約されることを記述したものですが、一方[S]にはそのような定義がありません。
...
(** For most function definitions over numbers, pure pattern
    ...
    keyword [Fixpoint]. *)
*)
(** 数値を扱う多くの関数は、単なるパターンマッチだけでは記述できず、再帰的な定義が必要になってきます。
...
*)
(** [beq_nat]関数は自然数を比較してbool値を返すものです。
...
*)
(** [blt_nat]関数は、自然数を比較して小さい、ということを調べてbool値を生成します（ [nat]ural numbers for [l]ess-[t]han）。
...
(** ここまでに、いくつかの型や関数を定義してきました。
    ...
    例えば、[0]が自然数の加算における左単位元（[0]が、左から加えても値が変わらない値であること）であることの証明は、[n]が何であっても[0 + n]を注意深く縮小(簡約)したものが[n]になることを、[+]という関数が「最初の引数を引き継いで再帰的に定義されている」ということを考慮した上で示せればいいということです。 *)
...
(** この定理と証明の様式は、以前示した例とほとんど同じですが、いくつか違いがあります。
    ...
    後者の違いは単なるスタイルの違いで、[Example]と[Theorem]（他にも[Lemma]、[Fact]、[Remark]など）はCoqから見るとすべて同じ意味を持ちます。
    ...
    [intros]や[simpl]、[reflexivity]は「タクティック(tactic)」の例です。
...
(** この定理は、あらゆる[n]や[m]について完全に成り立つと言っているわけではなく、[n = m]が成り立つときに限って成立する、というものです。
    ...
    また、同じように[n = m]という仮定を置く必要があります。
    [intros]タクティックはこれら三つを全てゴールから仮定の文脈に移動します。

    [n]と[m]が両方とも任意の数なのですから、これをこれまでの証明でやってきたように簡約することはできません。
    その代わりに、[n = m]ならば、イコールの両側の[n]や[m]を互いに書き換えても等しさは変わらない、というところに注目します。
...
*)
(** 証明の1行目は、∀（forall）がついた、つまり「あらゆる[n],[m]について」の部分をコンテキストに移しています。
    ...
    単に左側を右側に置き換えているだけです。
    逆に右側を左側に置き換えたい場合は、[rewrite <-]と書けます。
...
(** もちろん、どんな命題でも簡単な計算だけで証明できるという訳ではありません。
    一般に、未知だったり仮定の（任意のbool、自然数、リストなど）値は、我々が検証しようとしている関数の先頭に記述され、それが簡約の邪魔をしてくれます。
    例えば、下のような命題をsimplタクティックだけで証明しようとすると、すぐに行き詰まってしまうでしょう。 *)
...
(** その原因は、beq_natと+の定義で、共に最初の引数が[match]に渡されていることです。
    ...
    今求められていることは、[n]を何らかの条件に分割し、先に進めそうな形にすることができないかを検討することです。
    もし[n]が[O]なら、[beq_nat (n + 1) 0]の結果を得ることはできます。
    もちろん結果は[false]です。
    しかしもし[n]が何かの[n']を使って[n = S n']と表せると考えても、我々は[n + 1]の値を得ることはできません。
    ただ、その式が一つの[S]で始まる（始まらないものは[O]にマッチする）ことに着目すると、[beq_nat]の結果を計算して値を求めることができます。
    その結果[beq_nat (n + 1) 0]は、やはり[false]になるでしょう。
...
(** The [destruct] generates _two_ subgoals, which we must then
    ...
    inverse. *)
*)
(** [destruct]タクティックは二つのサブゴールを作ります。
    その両方を別々に、Coqを使って定理として証明していくことになります。
    一つのサブゴールからもう一つへ移動するための特別なコマンドは必要ありません。
    一つ目のサブゴールが証明されれば、それは消えて自動的にもう一つのサブゴールにフォーカスが移ります。
    この証明では、二つに分かれたサブゴールのいずれも[reflexivity]を1回使うだけで簡単に証明できます。

    destructについている注釈"[as [| n']]"は、"イントロパターン"と呼ばれるものです。
    これはCoqに対して、両方のサブゴールに元[n]だった変数をどのような変数名を使って取り入れるかを指示するものです。
    一般的に[[]]の間にあるものは"名前のリスト"で、"[|]"によって区切られます。
    このリストの最初の要素は空ですが、これは[nat]の最初のコンストラクタである[O]が引数をとらないからです。
    二つ目のコンストラクタ[S]は引数を一つ取りますので、リストの二つ目の要素である[n']を名前に使用します。

    [destruct]タクティックは帰納的に定義された型に対して使用できます。
    例えば、bool値の否定が対合であること・・・つまり否定の否定が元と同じになることを証明してみましょう。 *)
...
(** ここで使われている[destruct]には[as]句がありませんが、ここで展開している[b]の型[bool]の二つのコンストラクタが両方とも引数をとらないため、名前を指定する必要がないのです。
    ...
    実際のところほとんどの場合[destruct]の[as]句は省略可能です。
    その際はCoqの側で自動的に変数名をつけてくれます。
    これは確かに便利なのですが、よくない書き方とも言えます。
    Coqはしばしば名前付けに混乱して望ましくない結果を出す場合があります。 *)
...
(** To get a concrete sense of this, find a way to write a sensible
    ...
    of this restriction. *)
*)
(** これを具体的に感じるため、（自然数の単純な関数でかまわないので）全ての入力で停止するが、Coqが制限のため受け入れないような[Fixpoint]の定義を書きなさい。 *)
