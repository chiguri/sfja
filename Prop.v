...
[[
  ...
  直観的には、全ての数が偶数でないことが原因でこの証明は失敗します。では実際には何が原因で失敗しているでしょうか。
...
(** これらのような [inversion] の使い方は最初はちょっと謎めいて思えるかもしれません。
    これまでは、 [inversion] は等号に関する命題に対して使い、コンストラクタから元のデータを取り出すためか、別のコンストラクタを区別するためにしか使っていませんでした。
    しかし、ここでは [inversion] が 帰納的に定義された命題に対する根拠を分析するためにも使えることを紹介しました。

    （もしかすると[destruct]がより適していると感じたかもしれません。
    もちろん[destruct]を使うことはできますが、[destruct]はしばしば有用な情報を消し去ってしまいます。
    また、[eqn:]修飾はこの場合役に立ちません。）
