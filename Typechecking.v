...
(* 訳注: そもそも上の ":" の前は Typechecking の間違いだと思われるので、それを前提に修正した *)
...
(** STLCの[has_type]関係は（あるコンテキストのもとで）項が型に属するという意味を定義します。
    ...
    項の形にそのまま従います。
    このことから、型付け規則を型チェック「関数」の節にそのまま変換することができます。
...
*)
(** そしてこれが型チェッカです。与えられた項の構造をたどって調べ、[Some T] または [None] を返します。
    部分項の型を調べるために再帰呼び出しをするたびに、結果についてパターンマッチをして、 [None] でないことを確認します。
    [tm_app]の場合はさらに、パターンマッチングで関数型の矢印の右側と左側を抽出します（そして関数の型が [Tarrow T11 T12] という形ではないときは失敗します）。 *)
