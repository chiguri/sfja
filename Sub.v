...
>>
   は型付けできません。
   なぜなら、これはフィールドが1つのレコードを引数としてとる関数に2つのフィールドを持つレコードが与えられている部分を含んでいて、一方、[T_App]規則は関数の定義域の型は引数の型に完全に一致することを要求するからです。
   （訳注：おそらく修正漏れと思われる。実際には2つのフィールドを持つ[Person]を要求している関数を、3つのフィールドを持つ[Student]相当のレコード値に適用しているため、型が不一致となる。）
   ...
   同じことを別の観点から見ると、より豊かなフィールドを持つレコードは、そのサブセットのフィールドだけを持つレコードと「任意のコンテキストで少なくとも同等の良さである」と言えるでしょう。
   ...
   型[T]の値が求められている任意のコンテキストで型[S]の値が安全に使えるとき、「[S]は[T]のサブタイプである」と言い、非形式的に [S <: T] と書きます。
...
(** サブタイプは多くのプログラミング言語で重要な役割を演じます。
    ...
    オブジェクト[o]のメソッド[m]を引数[a1..an]のもとで呼ぶことは、[o]から[m]フィールドを射影として抽出して、それを[a1..an]に適用することです。
    ...
    サブクラス（またはサブインターフェース）のオブジェクトをスーパークラス（またはスーパーインターフェース）のオブジェクトの場所で使えるという事実は、複雑なライブラリの構築にきわめて便利なほどの柔軟性を提供します。
    ...
    クラスはまた、オブジェクトを構成しそのメソッドをインプリメントするのに使われる「コード」を与えます。
...
(** まず最初に、2つの「構造規則」("structural rules")を追加します。
    ...
    推移規則(rule of _transitivity_)は、直観的には、[S]が[U]よりも良く、[U]が[T]よりも良ければ、[S]は[T]よりも良いというものです。
<<
                              S <: U    U <: T
                              ----------------                        (S_Trans)
                                   S <: T
>>
    そして反射規則(rule of _reflexivity_)は、任意の型はそれ自身と同じ良さを持つというものです。
<<
                                   ------                              (S_Refl)
                                   T <: T
>>
...
[[
    ...
    そして、レコードのサブタイプについてはまだ議論していませんが、[Student] は [Person] のサブタイプであるとします。
...
*)
(** レコード型のサブタイプについての基本的直観は、「より小さな」レコードの場所で「より大きな」レコードを使うことは常に安全だということです。
...
>>
   ...
   ここでは、3つのより簡単な規則に分解します。この3つを[S_Trans]を使って結合することで、同じように作用させることができます。 *)
...
(** よりおもしろい例のために、[String]、[Float]のような、任意の「基本型」の集合を許します。
    これらの型の定数を追加したり、その型の上の操作を追加したりすることをわざわざやりませんが、そうすることは簡単です。 *)
...
*)
(** この章の残りでは、レコード型は省略して基本型、ブール型、関数型、[Unit]と[Top]のみ形式化し、直積型は練習問題にします。 *)
...
MoreStlcの章にある「レコードのエンコーディング」の項で、どのようにレコードを対で表現したか思い出しましょう。
...
(** これらは反転補題(_inversion lemma_)と呼ばれます。
    これは、後の証明で組込みの[inversion]タクティックと同じ役目をするためです。
    つまり、サブタイプ関係の主張 [S <: T] の導出が存在するという仮定と、[S]や[T]の形についてのいくつかの制約が与えられたとき、それぞれの補題は、[S]と[T]の形、および両者の構成要素間のサブタイプ関係の存在について、より多くのことが言えるためには、[S <: T] の導出がどういう形でなければならないかを提示するからです。 *)
...
(** 「定理」（進行）: 任意の項[t]と型[T]について、もし [empty |- t : T] ならば[t]は値であるか、ある項[t']について [t ==> t'] である。
    ...
    - 型付け導出の最後のステップで規則[T_App]が使われた場合、項[t1]、[t2]と型[T1]、[T2]が存在して [t = t1 t2]、[T = T2]、[empty |- t1 : T1 -> T2]、[empty |- t2 : T1] となる。
      ...
      - 最後に[t1]と[t2]がどちらも値とする。
        補題[canonical_forms_for_arrow_types]より、[t1]はある[x]、[S1]、[s2]について[\x:S1.s2]という形である。
...
(** 保存定理の証明はサブタイプを追加したことでやはり少し複雑になります。
    その理由は、上述の「サブタイプの反転補題」と同様に、純粋なSTLCでは「定義から自明」であった（したがって[inversion]タクティックからすぐに得られた）性質が、サブタイプの存在で本当の証明を必要とするようになったためです。
    サブタイプがある場合、同じ[has_type]の主張を導出するのに複数の方法があるのです。
 
    以下の「反転補題(inversion lemma)」は、もし関数抽象の型付け主張 [Gamma |- \x:S1.t2 : T] の導出があるならば、その導出の中に本体[t2]の型を与える部分が含まれている、ということを言うものです。 *)
