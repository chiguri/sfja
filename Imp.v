...
(** A minor technical point: Instead of asking Coq to import our
    ...
    relevant theorems. *)
*)
(** マイナーな技術的ポイント: ここまでの定義を[Logic]の章からインポートする代わりに、[Sflib.v]という小さなライブラリをインポートします。
    このライブラリは、前の章の定義や定理のうち、残りの章で実際に使うものだけを集めたものです。
    読者はそれほど違うものとは感じないでしょう。
    というのは、Sflib で抜けているもののほとんどは、Coqの標準ライブラリの定義と同じものだからです。
    こうする主な理由は、Coqのグローバルな環境を整理して、例えば、関連する定理を探すのを容易にするためです。 *)
...
(** この章では、プログラマが実際に書く具象構文から抽象構文木への変換は省略します。
    例えば、文字列["1+2*3"]をAST(Abstract Syntax Tree, 抽象構文木)[APlus (ANum 1) (AMult (ANum 2) (ANum 3))]にする変換のことです。
    この変換ができる単純な字句解析器と構文解析器をオプションの章[ImpParser]で実装します。
    このファイル（[Imp.v]）を理解するには[ImpParser.v]の理解は必要ではありませんが、もしそれらの技術についてのコース（例えばコンパイラコース）を受講していないならば、ざっと見てみるのも良いでしょう。 *)
...
(** 上述のCoq版と比較して...
       ...
       - BNFはより非形式的です。
         例えば、BNFは式の表面的な構文についていくらかの情報を与えています（可算は[+]と記述され、それは中置記号であるという事実などです）が、字句解析と構文解析の他の面は定めないままになっています（[+]、[-]、[*]の相対的優先順位などです）。
         （例えばコンパイラを実装するときに）この記述を形式的定義にするためには、追加の情報、および人間の知性が必要でしょう。
...
(** 前の証明の最後の繰り返しはちょっと面倒です。
    証明対象の言語や算術式や最適化が今に比べて著しく複雑だったら、現実的な問題になるでしょう。
...
*)
(** [T]がタクティックのとき、タクティック [try T] は[T]と同様ですが、[T]が失敗するとき[try T] は（失敗せずに）成功として何もしない点が違います。 *)
...
*)
(** 一番よく使われる形として、[;] タクティカルは二つのタクティックを引数として取ります。
    [T]と[T']がタクティックのとき、[T;T'] はタクティックで、最初に[T]を行い、[T]が生成したそれぞれのサブゴールに[T']を行います。 *)
...
[[
        ...
        [a1]のとり得る形を考える。そのほとんどの場合、[optimize_0plus]は部分式について単に自分自身を再帰的に呼び出し、[a1]と同じ形の新しい式を再構成する。
...
(** This proof can still be improved: the first case (for [a = ANum
    ...
    it looks: *)
*)
(** この証明はさらに改良できます。最初の場合（[a = ANum n] のとき）はかなり自明です。
...
*)
(** [induction]や[destruct]で、ほとんどの場合を一度に扱えるのはとても便利ですが、またちょっと混乱もします。
    よく起こる問題は、このスタイルで記述された証明をメンテナンスすることが難しいということです。
    例えば、後で、[aexp]の定義を拡張して、やはり特別な引数をとるコンストラクタを追加したとします。
    このとき上述の証明は成立しなくなっているでしょう。
    なぜなら、Coqは[APlus]についてのサブゴールの前にこのコンストラクタに対応するサブゴールを生成し、その結果、[APlus]の場合に取りかかる時には、Coqは実際にはまったく別のコンストラクタを待っていることになるからです。
    ここで欲しいのは、「この場所で[AFoo]の場合を期待していましたが、証明スクリプトは[APlus]について話しています。」という賢いエラーメッセージです。
    以下は、これを難なく可能にするいいトリックです（Aaron Bohannonが提案してくれました）。 *)
...
[[
    ...
    と書くと[a]についての帰納法を実行し（単に [induction a] と書いたのと同じです）、そして、「その上に」、[induction]によって生成されたそれぞれのサブゴールに[Case]タグを付加します。
    このタグは、そのサブゴールがどのコンストラクタから来たかのラベルです。
...
(** 最後に、役に立ちそうないろいろなタクティックをいくつか紹介します。
     ...
     - [subst x]: コンテキストから仮定 [x = e] または [e = x] を発見し、[x]をコンテキストおよび現在のゴールのすべての場所で[e]に置き換え、この仮定を消去します。
...
(** [aeval]と[beval]を[Fixpoints]によって定義された関数として示しました。
    ...
    この方法は、多くの場合[Fixpoints]を用いたものより柔軟です。
...
(** 始めに、プログラム変数などの識別子(_identifiers_)を形式化しなければなりません。
    このために文字列を使うこともできるでしょう。
    他にも、実際のコンパイラではシンボルテーブルへのポインタのようなある種の特別な構造を使うこともできるでしょう。
...
(** (We hide this section in a module because these definitions are
    ...
    can explain them.) *)
*)
(** （このセクションをモジュールに隠します。それは、これらの定義が実際には[SfLib]にあるからです。
    しかし説明のためにここで繰り返します。） *)
...
*)
(** 算術とブールの評価器は、自明な方法で変数を扱うように拡張できます: *)
...
*)
(** さて、Imp コマンド（「文(statement)」と呼ばれることが多い）の構文と挙動を定義する準備が出来ました。 *)
...
(** いつものとおり、より読みやすいよう、いくつかの [Notation] 宣言が使えます。
    しかし、Coq の組み込みの表記と衝突しないよう、少し気をつける必要があります （手軽さを維持しつつ！）。
    特に、[aexp] と [bexp] については、すでに定義した数値演算子やブール演算子との混同を避けるために、新しい表記は導入しません。
    同様の理由により、条件文に対しては通常使われる [IF] の代わりに [IFB] というキーワードを使います。 *)
...
*)
(** これは重要な変更です。
    評価を関数として定義するために必要な厄介な回避策から解放されるばかりでなく、定義での柔軟性も与えてくれます。
...
(** [ceavl] 関係に対する表記として [c / st || st'] を使います。
    正確に言うと、[c / st || st'] と書いたらプログラム [c] を初期状態 [st] で評価すると、その結果は最終状態 [st'] になる、ということを意味します。
...
(** Changing from a computational to a relational definition of
    ...
    Here's the proof: *)
*)
(** 評価の定義を計算的なものから関係的なものに変更するのは、評価は全関数であるべきという（Fixpointの定義におけるCoqの制限によって課せられる） 不自然な要求から逃れさせてくれる良い変更です。
    しかしこれは、2つ目の評価の定義は本当に部分関数なのか？という疑問ももたらします。
    つまり、同じ状態 [st] から始めて、あるコマンド [c] を違った方法で評価し、2つの異なる出力状態 [st'] と [st''] に至るのは可能か？ということです。
...
(** スタック言語のプログラムを評価するための関数を書きなさい。
    入力として、状態、数のリストとして表現されたスタック（スタックの先頭要素はリストの先頭）、および命令のリストとして表現されたプログラムを受け取り、受け取ったプログラムの実行した後のスタックを返します。
