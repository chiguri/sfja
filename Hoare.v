...
(** この方法で表明を書くことは、2つの理由から、若干ヘビーに見えます。
    ...
    (2)状態[st]は変数を参照するために使うただ1つのものです（2つの別々の状態を同時に考える必要はありません）。
...
(** ホーア論理のゴールは、ホーアの三つ組の正しさを証明する"合成的"手法を提供することです。
    つまり、プログラムの正しさの証明の構造は、プログラムの構造をそのまま反映したものになるということです。
    このゴールのために、以降の節では、Impのコマンドのいろいろな構文要素のそれぞれ対して、その構文要素について推論するための規則を1つずつ導入します。
    代入に1つ、コマンド合成に1つ、条件分岐に1つ、等です。
...
[[
    ...
    日本語で言うと、[Y]の値が[1]である状態から始めて、[X]を[Y]に代入するならば、[X]が[1]である状態になる、ということです。
...
[[
    ...
    さらに一般化して、「任意の」数についての性質[Q]が[X ::= a]の後に成り立つには、[X ::= a]の前で、[Q]内の「出現している全ての」[X]を[a]に置き換えたものが成り立っている必要があります。
...
[[
    ...
    はそうではないのです。この三つ組も正しいのですが、[hoare_asgn] （または [hoare_asgn_eq]）のインスタンスではないのです。
    なぜなら、[True] と [(X = 3) [X |-> 3]] は、構文的に等しい表明ではないからです。
    しかし、これらが論理的に等価である以上、一方の三つ組が正しいのであれば、もう一方も同様に正しくあるべきです。
...
(** ここで、良い機会ですので、Coq の別の便利な機能を紹介しておきましょう。
    ...
    なぜなら、すぐ次にやること、つまり[hoare_asgn]規則を適用することが、まさに、それがどうでなければならないかを決定することだからです。    
    こういう場合、[apply]の代わりに[eapply]を使うことができます。
    そうすることは、本質的に、「抜けている部分は後で埋めます」と Coq に伝えることになります。*)
