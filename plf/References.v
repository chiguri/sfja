...
(** ここまでは、いろいろな「純粋な」(_pure_)言語機能を考えてきました。
    ...
    その言語の中にはHaskellのような純粋な関数型言語、MLのような「ほとんど関数型の("mostly functional")」言語、Cのような命令型言語、JavaやC[#]、Scalaのようなオブジェクト指向言語を含みます。
    ...
    主要な拡張は、「記憶(_store_、あるいはヒープ(_heap_))」、そして位置を表す「ポインタ(_pointer_)」を明示的に扱うことです。
    この拡張は直接的に定義できます。
...
(** ほとんどすべてのプログラミング言語が、記憶に以前に置かれた内容を変更する何らかの代入操作を持っています。
    ...
    [y]を使って、[y]が指すセルに別の値を代入することが（[y:=84] と書くことで）できます。
    しかし、[y]を[+]のような操作の引数として直接使うことはできません。
...
(** 参照についての基本操作はアロケート(_allocation_)、逆参照(_dereferencing_)、代入(_assignment_)です。
       ...
       - 参照をアロケートするには、[ref]演算子を使います。
         ...
         例えば [ref 5] は値[5]を格納した新しいセルを生成し、そのセルへの参照に簡約されます。
 
       - セルの現在の値を読むためには、逆参照演算子[!]を使います。
         例えば [!(ref 5)] は [5] に簡約されます。
...
(** 参照セルの中身は数値でなければならないわけではありません。
    ...
    その任意の型の中には関数型も含まれます。
    例えば、関数への参照を使って、数値の配列の（非効率的な）実装をすることができます。
...
(** ここで定義した参照と、C言語スタイルの変更可能な変数には最後にもう一つ大きな違いがあります。
    ...
    nullポインタはC似の言語では大きな問題を引き起こします。
    どのポインタもnullになり得るため、実行時の逆参照は常に失敗する可能性を秘めているためです。
 
    MLのような言語でも、時には正しいポインタでないこともあるような場面は発生し得ます。
...
(** 参照の形式化に移る前に述べておくべき最後の問題が、記憶のデアロケーション(_de_-allocation)です。
    ...
    ガベージコレクションはプログラムから到達しなくなったセルを自動で判定し再利用するものです。        
 
    これは言語デザインの上で単なる趣味の問題ではありません。
    明示的なデアロケーション操作が存在した場合、型安全性を保つのが極度に困難になるのです。
    その理由の一つはよく知られたダングリング参照(_dangling reference_)問題です。
...
(** 参照の扱いについての一番巧妙な面は、操作的振る舞いをどのように形式化するかを考えるときに現れます。
    ...
    ほとんどのプログラミング言語の実装では、実行時の記憶は本質的には大きなバイト(byte)列です。
    ...
    このセグメントが「使用中」であることをどこかに保存して、新しくアロケートされた領域のインデックス（典型的には32ビットまたは64ビットの整数）を返却します。
    参照とはこのインデックスのことです。
...
>>
    ...
    さて、[ref]式の簡約結果は新しい場所です。これが項の構文と値の集合に場所を含めた理由です。
    これは重要なことですが、項の構文のこの拡張は、「プログラマ」が直接、具体的場所を含む項を書くことを意図したものではありません。
    そのような項は簡約の中間結果だけに現れます。
    これは奇妙に見えるかもしれません。
    しかしこの考え方は、簡約のすべてのステップの結果を項の変形で表現するという設計判断から自然に得られます。
    もし、より「機械に近い("machine-like")」モデル、例えば束縛された識別子の値を格納するのにスタックを利用するモデルを選んでいたならば、値の集合に場所を追加するアイデアはおそらくもっと自明だったでしょう。
...
>>
    ...
    これらの簡約規則はどのようなガベージコレクションも行わないことに注意します。
    単に簡約が進むにつれて記憶が限りなく大きくなることとします。
    これは簡約結果の正しさには影響しません（なにせ「ガベージ」の定義は、まさに記憶のもはや到達できない部分なので、簡約で何の影響も与えられません）。
...
(** 参照に対応するように構文と簡約規則を拡張したので、最後の仕事は新しい言語要素に対する型付け規則を書き下すこと（そして、もちろん、それが健全であることをチェックすることも！）です。
    ...
    何よりもまず、プログラマが実際に書く項の型チェックの目的のためには、この問題は現れないことに注意しましょう。
    具体的な場所定数は簡約の中間結果の項にのみ現れます。
    プログラマが書く言語の中には含まれません。
    すると、場所の型を決める必要があるのは、簡約列の途中にある時、例えば、進行補題や保存補題を適用しようとする時だけです。
    そのため、型付けは通常プログラムの「静的」性質と考えますが、場所の型付けがプログラムの「動的」状況にも依存するのは道理です。
 
    まず、具体的場所を含む項を簡約するとき、結果の型は開始時の記憶の中身に依存することに注意します。
    例えば、 記憶が [[unit, unit]] のときに項 [!(loc 1)] を簡約するならば、結果は[unit]です。
    記憶が [[unit, \x:Unit.x]] のとき同じ項を簡約すると、結果は [\x:Unit.x] です。
...
<<
                            ...
                            ---------------------------- 
                             Gamma |- loc l : Ref T1 
>>
    つまり、場所[l]の型を見付けるために、記憶内の[l]の現在の中身を探索し、その中身の型[T1]を計算するものです。
    するとこの場所の型は [Ref T1] になります。
 
    この方法から始めましたが、整合性のとれる状態に逹するまでもうちょっと進んでみる必要があります。
    事実上、記憶に依存した項の型を作ることで、型付け関係を、（コンテキスト、項、型の間の）3項関係から（コンテキスト、記憶、項、型の間の）4項関係に変更したことになります。
    記憶は、直観的には、項の型を計算するコンテキストの一部であることから、この4項関係を書くとき、記憶を[|-]記号の左側において、[Gamma; st |- t : T]のように書くことにしましょう。
...
>>
    ...
    しかしながら、この規則ではうまくいきません。
    1つに、型チェックがかなり非効率的になる点があります。
    ...
    またまずいことに、この型付け規則は、もし記憶に「循環(_cycle_)」を含んでいるとき何も導出できません。
    例えば、記憶：
<<
   [\x:Nat. (!(loc 1)) x, \x:Nat. (!(loc 0)) x]
>>
...
(** これらの問題はどちらも、上記の場所についての型付け規則が、項の中に記述があるたびに場所の型を再計算することが必要であるという事実から生じます。
    ...
    インデックス[i]の型はセル[i]に格納されうる値の型を記録したものです。 *)
...
(** もちろん、これらの型付け規則が簡約結果を正確に予言できるのは、
    ...
    そして、型保存定理より、この閉じた項は、もし必要なら型[T]の最終結果に簡約されます。
    以降、記憶と記憶型付けについての同様の直観をどのように形式化するかを見ます。
 
    しかしながら、プログラマが実際に書く項の型チェックをするには、どのような記憶型付けを使うべきかを頑張って推測する必要はありません。
    具体的場所定数は、評価の中間結果の項にのみ現れ、プログラマが書く言語には現れません。
    よって、プログラマが書く項は、「空の」記憶型付けによって型チェックできます。
    簡約が進み新しい場所が生成される場合も、新しくアロケートされたセルに置かれた初期値の型を見ることで、記憶型付けをどのように拡張したら良いかがわかります。
...
(** 保存定理は、新しい項[t']の型付けができる、[ST]を拡張する（つまり、すべての古い場所の値について[ST]と一致する）「何らかの」記憶型付け[ST']が存在することを主張するだけであることに注意します。
    ...
    上記のより弱いバージョンでも繰り返し「クランクを回す」のに適切な形をしており（なぜなら結論が仮定を含意するため）、すべての簡約ステップの「列」が型付け可能性を保存することが導かれます。
...
>>
   ...
   無限ループを開始するために、この関数を [(! r) unit] に適用することで実行を開始します。
