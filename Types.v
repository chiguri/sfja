...
(** 次に取り組む内容は型システムです。
    ...
    まずは、ブール値と数のみから成る言語から始め、型に関する基本的な考え方や型付け規則、型保存（type preservation）や進行（progress）といった型システムに関する基礎的な定理を導入します。
    その次に単純型付きラムダ計算に移ります。
...
(** 型システムについての議論を始めるために、例のごとく、ごく単純な言語から始めましょう。
    この言語は、実行時の型エラーで「まずいことが起きる」可能性のあるものであってほしいので、 [Smallstep] 章で使った、定数と足し算だけの言語よりはもう少し複雑なものでなければなりません。
...
(** この言語の [step] 関係について、まず注目に値するのは、 Smallstep 章の強進行性定理（strong progress theorem）が成り立たないということです。
    すなわち、正規形ではあるのに（これ以上簡約できないのに）、値ではない項があるのです（これは、そのような項を可能な「評価結果」と定義しなかったからです）。
    そのような項は [stuck] します。 *)
...
(** ただし、この言語では値の集合と正規形の集合は同一ではありませんが、値は正規形に含まれます。
    これは重要なことで、さらに簡約できる値を定義しまうことはできないことを表しています。 *)
...
(** ヒント: 証明中で、数値であることがわかっている項に対して帰納的推論をしなければならないことになります。
    ...
    どちらでも証明は進められますが、片方はもう片方よりもかなり短かくなります。
...
*)
(** 次にこの言語から見て取れる非常に重要なことは、行き詰まる項があったとしても、そのような項は、我々としても意味を持ってほしくないようなブール値と数の取り混ぜ方をしたもので、すべて「意味がない」ということです。
...
(** 非形式的な記法では、型付け関係を [|- t \in T] と書き、「[t] の型は [T] である」と読みます。
   ...
   （後の章ではターンスタイルの左側に追加の「コンテキスト」引数のある型付け関係もあります。ここでは、コンテキストは常に空です。） *)
(*
(** 
                           ----------------                            (T_True)
                           |- true \in Bool
...
(** 型付け関係には重要な性質がふたつあります。
    最初のものは、型のついた（well-typed）正規形は値である（行き詰まらない）、ということです。 *)
...
(** 型付けの第二の重要な性質は、型のついた項を一段階簡約すると、簡約結果もまた型のつく項である、ということです。
    ...
    この定理は主部簡約（subject reduction）性と呼ばれることが多々あります。
...
(** Coq でプログラミング言語の定義を扱っていると、ある具体的な項がどのように簡約されるか知りたいことがよくあります。
    ...
    このような証明は単純ですが、手でやるには繰り返しが多過ぎます。
...
(** We repeatedly apply [multi_step] until we get to a normal
    ...
    them. *)
*)
(** 正規形になるまで [multi_step] を繰り返し適用します。
    証明の途中に出てくる部分は、適切なヒントを与えてやれば [auto] で解けそうです。 *)
...
(** 下の [Tactic Notation] 定義はこのパターンを表現したものです。
    それに加えて、 [multi_step] を実行する前にゴールを表示します。
    これは、項がどのように評価されるか利用者が追えるようにするためです。 *)
...
[] *)
*)
(** 主部簡約性が成り立つのなら、その逆の性質、主部展開（subject expansion）性も成り立つかどうか考えるのが合理的でしょう。
...
[] *)
*)
(** 評価規則 [E_PredZero] には少し直感に反するところがあります。
