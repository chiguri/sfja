...
html: $(GLOBFILES) $(VFILES)
	...
	--no-lib-name --lib-subtitles -toc --toc-depth 2 -html -d html $(VFILES)
	cp index.html coqdoc.css html
	cp deps.html deps.gif jquery.maphilight.min.js html
	tar zcvf html/sfja.tar.gz *.v *.css *.js *.html Makefile README README_TRANSLATION LICENSE *.gif impdriver.ml normdriver.ml
...
.PHONY: all archclean beautify byte clean cleanall gallina gallinahtml html install install-doc install-natdynlink install-toploop opt printenv quick uninstall userinstall validate vio2vo html-normal
...
#	find . -name .coq-native -type d -empty -delete
	...
	rm -f $(VOFILES:%.vo=.%.aux) $(VOFILES:%=.%.aux)
