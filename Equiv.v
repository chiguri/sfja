...
(** *** 宿題割当てについての一般的アドバイス
    ...
    - 仕事を減らすために自動化を使いなさい。
      この章の練習問題の証明のいくつかは、もしすべての場合を明示的に書き出すとすると、とても長くなります。*)
...
*)
(** 前の章で、簡単なプログラム変換の正しさを調べました。[optimize_0plus]関数です。
    対象としたプログラミング言語は、算術式の言語の最初のバージョンでした。
    それには変数もなく、そのためプログラム変換が正しいとはどういうことを意味する(_means_)かを定義することはとても簡単でした。
...
(** 変数が入っている[aexp]と[bexp]については、どう定義すれば良いかは明らかです。
    2つの[aexp]または[bexp]が振る舞い同値である(_behaviorally equivalent_)とは、「すべての状態で」2つの評価結果が同じになることです。*)
...
(** コマンドについては、状況はもうちょっと微妙です。
    ...
    コマンドによっては（特定の状態から開始したときには）停止しないためどのような状態にもならないことがあるからです！
    すると次のように言う必要があります。
    2つのコマンドが振る舞い同値であるとは、任意の与えられた状態から両者をスタートすると、両者ともに発散するか、両者ともに停止して同じ状態になることです。
...
*)
(** しかしここで行き詰まります。
    ゴールは合理的に見えますが、しかし実は証明できません！
    前の章で[update]に関して証明した補題のセットをふりかえってみましょう。
    補題[update_same]がほとんどのことをやっていますが、全部ではありません。
    [update_same]は元の状態と更新された状態がすべての値について一致することを言っていますが、それは、Coqの意味で[eq]だと言っているわけではありません! *)
...
(** 何がどうなっているのでしょう？
    ...
    Coqでは、関数同士が等しいとは、その定義が簡単化(simplification)の範囲での変形を除いて構文的に同じということです。
    （簡単化だけが Coq で[eq]コンストラクタに適用することが許されたものなのです!）
    実際には、[update]操作の繰り返しで構築された関数については、2つの関数が等しいと証明できるのは「同じ」[update]操作を同じ順番で適用した場合だけです。
    上述の定理では、第一パラメータ[cequiv]のupdateの列は第二パラメータのものより1つ長いので、等しさが成立しないのも当然です。 *)
...
[[
    ...
    つまり、すべての入力に対して同一の振る舞いをする2つの関数が出てくるのですが、その2つが[eq]であることが証明できないのです。

    こういった状況でこれから使おうとしている推論原理は、関数外延性(_functional extensionality_)と呼ばれます:
[[
                        forall x, f x = g x
                        -------------------
                               f = g
]]
    この原理は Coq のビルトインの論理からは導出できませんが、追加の公理(_axiom_)として導入することは安全です。*)
...
(** Coq にこの公理を導入することが矛盾を生むものではないことを示すことができます。
    （このように、この公理の導入は、[excluded_middle]のような古典論理の公理を追加するのと同様なのです。） *)
...
(** 概して、この明示的な同値のアプローチは、関数外延性を使うものよりかなり難しくなります。
    ...
    その方法は、同値関係をシステムに登録すると、それを使った書き換えタクティックが、もとの等しさ(equality)に対してとほとんど同じようにはたらくようになるというものです。）
