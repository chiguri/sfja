...
(** ここまでは、いろいろな「純粋な」(_pure_)言語機能を考えてきました。
    ...
    その言語の中にはHaskellのような純粋な関数型言語、MLのような「ほとんど関数型の("mostly functional")」言語、Cのような命令型言語、Javaのようなオブジェクト指向言語を含みます。

    ほとんどの実際のプログラミング言語は、ここまで使ってきた単純な意味論の枠組みでは記述できない様々な「不純な(_impure_)」機能も持っています。
    特に、これらの言語では項を評価することで、単に結果を得る他に、変更可能な変数（あるいは参照セル、配列、変更可能なレコードフィールド、等）に代入したり、ファイルや画面やネットワークに入出力したり、
    例外やジャンプ、継続によってローカルな枠を越えて制御を移したり、プロセス間の同期や通信を行ったりします。
    プログラミング言語についての文献では、これらの計算の副作用("side effects")はより一般に計算作用(_computational effects_)と参照されます。

    この章では、ここまで学習してきた計算体系に一つの計算作用「変更可能な参照」を追加する方法を見ます。
...
(** ほとんどすべてのプログラミング言語が、記憶に以前に置かれた内容を変更する何らかの代入操作を持っています。
    ...
    いくつかの言語（特にMLやその親戚）では、名前束縛の機構と代入の機構を区別しています。
    「値」として数値[5]を持つ変数[x]を持つことも、現在の内容が[5]である変更可能なセルへの参照(_reference_)またはポインタ(_pointer_)を値とする変数[y]を持つこともできます。
    ...
    その代わり、現在の内容を得るために明示的に参照を手繰る(_dereference_、逆参照する)ことが必要です。
    これを[!y]と書きます。

    他のほとんどの言語、特にJavaを含むCファミリーのメンバーのすべてでは、すべての変数名は変更可能なセルを指します。
...
(** 参照についての基本操作はアロケート(_allocation_)、逆参照(_dereferencing_)、代入(_assignment_)です。
       ...
       - 参照をアロケートするには、[ref]演算子を使います。
         ...
         例えば [ref 5] は値[5]を格納した新しいセルを生成し、そのセルへの参照に評価されます。
...
*)
(** 直観的には...
    - [ref t] （形式的には [tref t]）は値[t]が格納された新しい参照セルをアロケートし、新しくアロケートされたセルの場所(location)を評価結果とします。
...
<<
                       ...
                       Gamma |- t1 := t2 : Unit
>>
    場所についての規則はもう少し仕掛けが必要になり、それが他の規則にいくらかの変更を求めることになります。
    これについては後にまた戻ってきます。 *)
...
*)
*)
(** 代入式の結果がつまらない値[unit]であるという事実によって、順次処理(_sequencing_)のうまい略記が使えます。
    例えば、
<<
       (\x:Unit. !r) (r := succ(!r)).
>>
...
>>
    ...
    1つ目の式の型を[Unit]に限定することで、1つ目の値を捨てることができるのは本当にそれがつまらない値であることが保証されているときだけになり、型チェッカで馬鹿なエラーをチェックするのに役立ちます。
...
*)
(** 形式的には、順次処理 [tseq] を"derived form"として導入します。
...
*)
(** [r]に束縛される参照(_reference_)と、この参照によって指されているセル(_cell_)の違いを心に留めておく必要があります。
...
>>
    ...
    別名を付けられる能力があることによって、参照を持つプログラムに関する推論は、きわめてトリッキーになります。
    例えば、式
<<
      r := 5; r := !s
>>
...
*)
(** ここで、それぞれ引数の型は[Unit]なので、[incc]と[decc]の定義において関数抽象は関数本体に特に有用な情報を提供しないことに注意します（束縛変数名にワイルドカード[_]を使っているのは、このことを合図したものです）。
    そうではなく、関数抽象の目的は関数本体の実行を「遅く」するためです。
    関数抽象は値であることから、2つの[let]は単に2つの関数を名前[incc]と[decc]に束縛するだけで、実際に[c]を増やしたり減らしたりはしません。
    後に、これらの関数の1つを呼び出すたびに、その本体が1度実行され[c]について対応する変更が行われます。
    こういった関数はしばしば _thunk_ と呼ばれます。

    これらの宣言のコンテキストで、[incc]を呼ぶと[c]が変更されますが、これは[decc]を呼ぶことで確認できます。
...
>>
    ...
    [update]関数は、配列、インデックス、そのインデックスの場所に格納する新しい値をとり、新しい関数を生成し（そしてそれを参照に格納し）ます。
...
(** ここで定義した参照と、C言語スタイルの変更可能な変数にはもう一つの違いがあります。
    ...
    そのような「nullポインタ」の逆参照はエラーで、例外になったり（Java）、プログラムが停止したり（C）します。

    Cのような言語ではnullポインタは重大な問題を起こします。
    任意のポインタがnullになる可能性があるという事実は、任意の逆参照操作が潜在的に失敗の可能性を持つということです。
    しかしMLのような言語でも、時には正しいポインタを持つことを許すことも許さないこともできるようにしたい場合があります。
...
(** 参照の形式化に移る前に述べておくべき最後の問題が、記憶のデアロケーション(_de_-allocation)です。
    ...
    数値を持つセルをアロケートし、何かのデータ構造にそれへの参照を持たせ、それをしばらく利用し、そしてそれをデアロケートし、ブール値を持つ新しいセルをアロケートします。
    このとき同じ記憶が再利用されるかもしれません。
    すると、同じ記憶セルに2つの名前があることになります。1つは [Ref Nat] 型で、もう1つは [Ref Bool] 型です。 *)
...
(** 参照の扱いについての一番巧妙な面は、操作的振る舞いをどのように形式化するかを考えるときに現れます。
    ...
    ほとんどのプログラミング言語の実装では、実行時の記憶は本質的にはバイト(byte)の（大きな）配列です。
    実行時システムは、この配列のどの部分が現在使用されているかを常に監視しています。
    新しい参照セルをアロケートしなければならないとき、記憶の自由範囲から十分な大きさのセグメント（整数セルには4バイト、[Float]のセルには8バイト、等）をアロケートします。
    このセグメントに「使用中」のマークをして、新しくアロケートされた領域のインデックス（典型的には32ビットまたは64ビットの整数）を返却します。
    参照とはこのインデックスのことです。
    ...
    この抽象度であることを強調するため、これ以降、用語として、「参照」や「ポインタ」の代わりに「場所(_location_)」を使います。

    この方法で場所を抽象的に扱うことで、Cのような低レベルの言語で見られる「ポインタ算術(_pointer arithmetic_)」をモデル化することができなくなります。
    この制約は意図的なものです。
    ポインタ算術は時には非常に便利です。
    特にガベージコレクタのような低レベルのサービスを実装するときなどです。
    しかし、ほとんどの型システムではポインタ算術を追跡することができません。
    記憶の場所[n]に[float]が格納されていることを知っても、場所[n+4]の型について何も意味のあることはわかりません。
...
(** IMPのスモールステップ操作的意味論では、ステップ関係は、実行されるプログラムに加えて補助的な状態を持ちまわる必要があったことを思い出して下さい。
    ...
    （この表現を以前には使えなかった理由は、IMPでは、プログラムはいつでも任意の場所を変更することができるので、状態はいつでも任意の変数を値に写像することができるようになっていないといけなかったからです。
    しかし、STLCに参照を追加したものでは、参照セルを作る唯一の方法は [tm_ref t1] を使うことです。
    ここで [tm_ref t1] は[t1]の値を新しい参照セルに置き、評価結果として新しく生成された参照セルの場所を返します。
...
(** 次に、操作的意味を記憶を考慮した形に拡張します。
    ...
    さらに、項の評価は記憶に副作用を起こし、それが後の別の項の評価に影響を及ぼすので、評価規則は新しい値を返す必要があります。
    これから、1ステップ評価関係の形は [t ==> t'] から [t / st ==> t' / st'] に変わります。
...
>>
    ...
    これは重要なことですが、項の構文のこの拡張は、「プログラマ」が明示的に具体的場所を含む項を書くことを意図したものではありません。
    そのような項は評価の中間結果だけに現れます。
    これは最初は奇妙に見えるかもしれません。
    しかしこれは、評価のすべてのステップの結果を変形された項で表現するという設計判断に、実に自然に馴染むのです。
    もし評価に対して、より「機械に近い」("machine-like")モデル、例えば束縛された識別子の値を格納する明示的なスタックを選んだならば、許される値の集合に場所を追加したアイデアはおそらくより明確になるでしょう。
...
>>
    ...
    この場合、評価規則は単に行き詰まります。
    後に確立する型安全性は、型付けがされた項がこの方法で間違った振る舞いをすることがないことを保証します。）
<<
                               l < |st|
                     ----------------------------------           (ST_DerefLoc)
                     !(loc l) / st ==> lookup l st / st
...
>>
    記法 [[v2/l]st] は「[l]を[v2]に写像し、他の場所は[st]と同じものに写像する記憶」を意味します。
...
>>
    ...
    これは評価結果の正しさには影響しません（なにせ「ガベージ」の定義は、まさに記憶のもはや到達できない部分なので、評価の上で以降何の影響も与えられません）。
    しかしこれは、洗練された評価器ならばガベージとなったものの場所を再利用して実行を続けることができる場面で、素朴な実装はメモリを使い果たす可能性があることを意味します。
...
*)
*)
(** 参照に適用するように構文と評価規則を拡張したので、最後の仕事は新しい言語要素に対する型付け規則を書き下すこと、そして、もちろん、それが健全であることをチェックすることです。
    自然と、キーとなる問題は「場所の型は何か？」になります。

    何よりもまず、プログラマが実際に書く項の型チェックの目的のためには、この問題に答える必要は「ない」ことに注意しましょう。
    具体的な場所定数は評価の中間結果の項にのみ現れます。
    プログラマが書く言語の中には含まれません。
    すると、場所の型を決める必要があるのは、評価列の途中にある時、例えば、進行補題や保存補題を適用しようとする時だけです。
    そのため、型付けは通常プログラムの「静的」性質と考えますが、場所の型付けがプログラムの「動的」状況にも依存するのは道理です。

    まず、具体的場所を含む項を評価するとき、結果の型は開始時の記憶の中身に依存することに注意します。
    例えば、 記憶が [[unit, unit]] のとき項 [!(loc 1)] を評価するならば、結果は[unit]です。
    記憶が [[unit, \x:Unit.x]] のとき同じ項を評価すると、結果は [\x:Unit.x] です。
    前者の記憶に関して、場所[1]は型[Unit]を持ち、後者の記憶に関しては型[Unit->Unit]を持ちます。
...
<<
                            ...
                            ----------------------------
                             Gamma |- loc l : Ref T1
>>
    つまり、場所[l]の型を見付けるために、記憶内の[l]の現在の中身を探索し、その中身の型を計算するものです。
    するとこの場所の型は [Ref T1] になります。

    この方法から始めましたが、整合的な状態に逹するまでもうちょっと進んでみる必要があります。
    事実上、記憶に依存した項の型を作ることで、型付け関係を、（コンテキスト、項、型の間の）3項関係から（コンテキスト、記憶、項、型の間の）4項関係に変更したことになります。
    記憶は、直観的には、項の型を計算するコンテキストの一部であることから、この4項関係を書くとき、記憶を[|-]記号の左側に書くことにしましょう:
<<
    [Gamma; st |- t : T].
>>
...
>>
    ...
    もし[l]が項[t]に何回も現れるときには、[t]の型導出を構成する過程の中で[v]の型を何回も再計算することになります。
    下手をすると[v]自体が場所を含んでいるかもしれません。
    すると毎回その型を計算しなければなりません。

    2つ目は、上記の場所についての型付け規則では、記憶が循環(_cycle_)を含んでいるとき何も導出できません。
    例えば、記憶：
<<
   [\x:Nat. (!(loc 1)) x, \x:Nat. (!(loc 0)) x]
>>
...
*)
*)
(** 2つの問題はどちらも、上記の場所についての型付け規則が、項の中に記述があるたびに場所の型を再計算することが必要であるという事実から生じます。
    しかしこれは、直観的に、必要であるべきではありません。
    とにかく、場所が最初に生成されたとき、そこに格納された初期値の型はわかっています。
    特定の場所に格納される値の型は「決して変化しない」という不変条件が定められていると仮定しましょう。
    つまり、この場所に後に別の値を格納することがあったとしても、その値は常に初期値と同じ型を持つ、ということです。
    言い換えると、記憶のすべての場所に対して、それがアロケートされたときに決まる1つの確定した型を常に記憶しているということです。
    すると、これらの意図した型を記憶型付け(_store typing_)としてまとめることができます。
    これは、場所を型に写像する有限関数です。

    通常通り、更新についてのこの「保守的」型付け制約は、行き詰まることなく完全に評価できるいくつかのプログラムを ill-typed として除外します。
*)
...
(** 記憶[st]を記述する記憶型付け[ST]が与えられ、そこである項[t]が評価されると仮定します。
    ...
    例えば、もし[ST]が [[Unit, Unit->Unit]] ならば、 [!(loc 1)] が型 [Unit->Unit] を持つとすぐに推論できるでしょう。
...
>>
    ...
    つまり、[l]が正しい場所である限りは（つまり[ST]の長さ未満の場合は）、[l]の型を[ST]から調べ上げるだけで計算することができます。
    型付けはやはり4項関係ですが、それは具体的記憶ではなく、 記憶型付けがパラメータになります。
...
*)
(** 評価関係と型付け関係の両者を拡張した（評価関係については初期記憶と最終記憶を、型付け関係については記憶型付けを）ことから、保存定理の主張は、これらのパラメータを含むように変えなければなりません。
...
(** If we typecheck with respect to some set of assumptions about the
    ...
    this.  *)
*)
(** もし記憶内の値の型についてのいくつかの仮定の上で型チェックを行い、その後、その仮定をやぶる記憶のもとで評価をしたならば、結果は悲惨なものになるでしょう。
...
(** 保存定理は、新しい項[t']の型付けができる、[ST]を拡張する（つまり、すべての古い場所の値について[ST]と一致する）「何らかの」記憶型付け[ST']が存在することを主張するだけであることに注意します。
    ...
    もちろん直観的には[ST']は、[ST]であるか、そうでなければ拡張された記憶 [snoc st v1] の値[V1]の型[T1]についての [snoc ST T1] であることは明らかです。
    しかしこれを明示的に述べようとすると、定理の主張がいたずらに複雑になります（これによってより有用になることは何もありません）。
    上記のより弱いバージョンでも繰り返し「クランクを回す」のに適切な形をしており（なぜなら結論が仮定を含意するため）、すべての評価ステップの「列」が型付け可能性を保存することが導かれます。
    この定理と進行性を組み合わせることで、「型付けできるプログラムはエラーにならない」という通常の保証が得られます。
...
(** 最後に、記憶型付けについての補題が必要です。
    ...
    証明論の弱化補題は、ある理論に新しい仮定を追加しても証明可能な定理が減ることはないことを言うものです。） *)
...
>>
   ...
   最初に、[ref (\x:Unit.unit)] が型 [Unit -> Unit] のセルへの参照を作ります。
   次にこの参照を、ある関数に引数として渡します。
   その関数は、引数を名前[r]に束縛し、そして引数に関数 (\x:Unit.(!r) unit) を代入するものです。
   ここで関数 (\x:Unit.(!r) unit) は、引数を無視して[r]に格納された関数を引数[unit]に適用するものです。
   しかしもちろん、[r]に格納された関数とは自分自身です!
   玉が転がり出すために、最後にこの関数を [(! r) unit] に適用することで実行を開始します。
*)
